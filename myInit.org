...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package general ; keybindings
    ...
    (general-define-key
      ...
      "en" '(ein:notebooklist-open :which-key "open notebooklist")
      "es" '(ein:jupyter-server-start :which-key "start jupyter server")
      "eq" '(ein:jupyter-server-stop :which-key "stop jupyter server")
